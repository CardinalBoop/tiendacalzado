...
$rutCliente=consultasSQL::clean_string($_POST['clien-rut']);
...
if(!$rutCliente=="" && !$nameCliente=="" && !$apeCliente=="" && !$dirCliente=="" && !$phoneCliente=="" && !$emailCliente=="" && !$fullnameCliente==""){
    if($passCliente==$passCliente2){
        $verificar= ejecutarSQL::consultar("SELECT * FROM cliente WHERE RUT='".$rutCliente."'");
        $verificaltotal = mysqli_num_rows($verificar);
        if($verificaltotal<=0){
            if(consultasSQL::InsertSQL("cliente", "RUT, Nombre, NombreCompleto, Apellido, Direccion, Clave, Telefono, Email", "'$rutCliente','$nameCliente','$fullnameCliente','$apeCliente','$dirCliente', '$passCliente','$phoneCliente','$emailCliente'")){
                echo '<script>
                    swal({
            ...
            echo '<script>swal("ERROR", "El RUT que ha ingresado ya está registrado en el sistema, por favor ingrese otro RUT", "error");</script>';
        }
